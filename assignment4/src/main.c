...
{
	//this is one subtraction and one division less
	double complex z=cmax-cmin;
	z=z/res;
	double real = ((double) x) * creal(z) + creal(cmin);
	double imag = ((double) y) * cimag(z) + cimag(cmin);
	z= real + imag * I;

	double complex oldz=z;
	unsigned long long dwell = 0;

	// Exit condition: dwell is maxDwell or |z| >= 4
	//by removing the function, and doing it inline, and by just having the first z value stored a lot fewer instuctions are needed
...
//the buffer does not need to be remeade for each iteration
//and having the the part of the loop iterating most often at the end of the 2d array index
//makes it faster
