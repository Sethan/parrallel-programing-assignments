...
{
	(void) id;
	//A local list of work to be done, this reduces the number of locks used, but increases somewhat wait time
	jobQueue *localHead=NULL;
	//If either the main thread still is uploading values or there is work to be done the threads will work
	while(stillWorking||jobQueueHead!=NULL)
	{
			//A thread locks the stack and gets ten jobs
			pthread_mutex_lock(&lock);
			for(int i=0;i<10;i++)
			{
				if(jobQueueHead!=NULL)
				{
						putJob(&localHead,  popJob(&jobQueueHead));
				}
			}
			//The stack is unlocked
			pthread_mutex_unlock(&lock);
			//While the thread has its own work to do its going to work trough its local list
			while(localHead!=NULL)
			{
				//A job is read, if it has a callback then its going to use marianiSilver function otherwise just computeBlock
				job current = popJob(&localHead);
				if(current.callback)
				{
					marianiSilver(current.dwellBuffer, current.atY, current.atX, current.blockSize);
				}
				else
				{
					computeBlock(current.dwellBuffer, current.atY, current.atX, current.blockSize);
				}
			}
...
void initializeWorkers(unsigned int threadsNumber, pthread_t *t) {
	(void) threadsNumber;
	...
	for(unsigned int i=0;i<threadsNumber;i++)
	{
			...
			pthread_create(&t[i],&attr,worker,&t[i]);
	}
					...
					unsigned int const blockSize)
{
	...
	{
		//locks stack creates job with no callback and the unlocks
		pthread_mutex_lock(&lock);
		createJob(NULL,buffer, atY, atX, blockSize);
		pthread_mutex_unlock(&lock);
		...
		for (unsigned int ydiv = 0; ydiv < subdivisions; ydiv++) {
			for (unsigned int xdiv = 0; xdiv < subdivisions; xdiv++) {
				//Locks stack and adds job with the callback 1, I was not to sure what the flag value should be so i did it this way
				//Unlocks the thread
				pthread_mutex_lock(&lock);
				createJob(1,buffer, atY + (ydiv * newBlockSize), atX + (xdiv * newBlockSize), newBlockSize);
				pthread_mutex_unlock(&lock);
			}
		}
	}
...
int main( int argc, char *argv[] )
{
	...
	//Main thread initializes the threads with a thread id array
	pthread_t *tid = malloc(sizeof(pthread_t)*useThreads);
	initializeWorkers(useThreads,tid);
	if (useMarianiSilver) {
	...
	//Main thread has added the first set of work to the stack
	stillWorking=false;
	//Main thread wait until other threads are done and joins them 
